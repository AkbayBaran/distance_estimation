...
# classes dosyasından veri çekme 
class_names = []
...
def object_detector(image):
    ...
    # nesneleri eklemek için bir boş liste 
    data_list =[]
    for (classid, score, box) in zip(classes, scores, boxes):
        # sınıf kimliğine göre renk atama işlemi  
        color= COLORS[int(classid) % len(COLORS)]
    
        label = "%s : %f" % (class_names[classid[0]], score)

        # tanımlanan nesneyi dikdörtgen içine almak 
...
# odak uzaklığını bulma 
focal_person = focal_length_finder(KNOWN_DISTANCE, PERSON_WIDTH, person_width_in_rf)

focal_mobile = focal_length_finder(KNOWN_DISTANCE, MOBILE_WIDTH, mobile_width_in_rf)
cap = cv2.VideoCapture(0)
fourcc = cv2.VideoWriter_fourcc(*'XVID')
Recoder = cv2.VideoWriter('out.mp4', fourcc,8.0,(int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))) )
while True:
    ret, frame = cap.read()
